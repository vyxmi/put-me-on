...
function NameNode({
  ...
  return (
    <button
      ...
      onBlur={() => onHover(null)}
      className="rounded-sm text-[15px] text-text-primary transition-colors hover:text-accent"
    >
      {label}
    </button>
  );
...
export function ChainPreview({ chain }: { chain: ChainLink }) {
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-2xl border border-border px-5 py-6">
      ...
      <div className="flex items-center gap-2">
        <Artwork seed={chain.track.artSeed} size={28} radius={7} />
